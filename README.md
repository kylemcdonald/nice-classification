...
The gear menu switches instantly among **Basic** (the original SVG marks), **Detailed** (the new single-object image set), and **Complex** (the first generated image set). Detailed is the default, and the browser remembers the selected style between visits. Both raster sets are used as masks so every mark retains its class-group color.
