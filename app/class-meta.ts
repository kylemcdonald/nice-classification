...
// Compact custom marks for objects that the core icon set does not represent
// plainly enough at the overview's small display size.
const mdiRubberRoll =
...
export type ClassMeta = {
  ...
  singleObjectImage: string;
};

const classDefinitions: Array<
  Omit<ClassMeta, "generatedImage" | "singleObjectImage">
> = [
  ...
  { shortName: "Textiles", icon: mdiCurtains, group: "materials" },
  { shortName: "Clothing", icon: mdiTshirtCrew, group: "lifestyle" },
  { shortName: "Trims & fasteners", icon: mdiSewOnButton, fillRule: "evenodd", group: "lifestyle" },
...
export const classMeta: ClassMeta[] = classDefinitions.map((definition, index) => {
  const file = `class-${String(index + 1).padStart(2, "0")}.png`;
  return {
    ...definition,
    generatedImage: `./class-icons/${file}`,
    singleObjectImage: `./class-icons-single/${file}`,
  };
});
